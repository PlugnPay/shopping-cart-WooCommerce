...
Tags: woocommerce cardx.com, cardx.com, payment gateway, woocommerce, woocommerce payment gateway
...
Accept payments on your WooCommerce website with CardX’s secure payment modules.

== Description ==

CardX SSv2 Payment Gateway for WooCommerce makes your website ready to use CardX payment gateway to accept payments on your ecommerce store in a safe way.
...
5. A safe way to process credit/debit cards on WooCommerce using CardX's Smart Screens v2 payment method.
6. Payment data is collected on CardX’s secured servers.
...
9. Under 'Payment Gateways' you will find all the available gateways, select 'CardX SSv2 Payment Gateway For WooCommerce' option
...
Authorization Hash: Select only if you require Authorization Verification Hash functionality. (Note: Merchant must configure these related settings to match their CardX account before activating.)

Authorization Hash Key: If using this ability, enter the corresponding verification key from your CardX account.

Authorization Hash Fields: If using this ability, select a fieldset to validate upon & configure your CardX account to match.
---------------------------------------------
